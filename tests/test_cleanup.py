...
def test_check_new_urls_respects_skip_and_generates_file_scheme(
    context_tmp, tmp_path, monkeypatch, write_csv
):
    ...
    files = {
        "https://old.example.com/1.jpg": models.ForumFile(
            fileid="1", url="https://old.example.com/1.jpg", result=models.FileResult.skipped
        ),
    ...
    assert cleanup.check_new_urls(context_tmp, files) is True
...
def test_check_old_urls_detects_in_updated_or_nonupdated(context_tmp, tmp_path, write_csv):
    ...
    ok = cleanup.check_old_urls(context_tmp, files_to_check, legacy=False)
    assert ok is False
...
def test_delete_files_batches_and_calls_client(context_tmp, monkeypatch):
    ...
    context_tmp.args = models.CliArgs(mode="delete_files", apply=True, yes=True)

    cleanup.delete_files(context_tmp)
    # Should batch at 100
    assert len(calls) == 3
